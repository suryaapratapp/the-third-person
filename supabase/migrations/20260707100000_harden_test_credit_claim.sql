-- Closes the "unlimited free credits" hole in claim_test_credit_pack.
-- Previously any authenticated user could call this repeatedly (e.g. from
-- devtools) and grant themselves unlimited paid-tier credits with no
-- payment behind them. This keeps the pre-checkout testing mechanism
-- (real Razorpay integration is a separate, later change) but caps it to
-- one claim ever per user, matching a normal "one free trial pack" limit.
